...
var orgWelcomeMail = fs.readFileSync(path.join(__dirname + 
                  '/../../views/mail/welcome_org.jade'), 'utf8');
...
exports.sendWelcomeMail = function(user) {
  ...
  if (user.type == 'institution_verified') {
    html = jade.render(orgWelcomeMail, {user: user});
    
  } else {
    html = jade.render(welcomeMail, {user: user});
