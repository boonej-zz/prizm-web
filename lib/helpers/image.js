...
var processImageRaw = function(i, p, fields, next, c, bucket){
  console.log('processingimage');
  bucket = bucket?bucket:AWS_BUCKET;
  var s3 = new AWS.S3();
  var path = p.path;
  var q = 1;
  if (c) {
  ...
  gm(i.path).size(function(err, d){
    console.log(err);
    if (d){
      var targetHeight = p.height;
      var targetWidth = p.width;
      var rat = d.width < d.height?targetWidth:targetHeight;
    var min = d.width < d.height?d.width:d.height;
    var ratio = targetWidth/targetHeight;
    var target = min/q;
    var cropWidth = d.width < d.height?d.width:d.height/ratio;
    var cropHeight = d.height < d.width?d.height:d.height/ratio;
    gm(i.path)
    .autoOrient()
    .gravity('Center')
...
var processImage = function(i, p, fields, next, c, bucket){
  console.log('processingimage');
  bucket = bucket?bucket:AWS_BUCKET;
  var s3 = new AWS.S3();
  var path = p.path;
...
exports.uploadMessageImage = function(req, next){
  ...
  var user = req.user;
  var date = moment().format('ssmmhhDDMMYYY');
  var ext = '.jpg';
  var path = date + uuid.v1() + '-message';
  var form = new multiparty.Form();
  var onComplete = function(err, path, fields){
    var url = 'https://s3.amazonaws.com/higheraltitude.prism/' +
      path;
    next(err, url, fields);
  };
  form.parse(req, function(err, fields, files) {
    var img = files.image;
    if (img && img.length > 0 && img[(img.length -1)].size > 0) {
      uploadImage(img[(img.length - 1)], path, ext, fields, onComplete);
    } else {
      console.log(fields);
      next(err, false, fields);
    }
  });
...
exports.uploadSettings = function(req, next){
  var uploadImage = function(img, path, ext, fields, next){
    var props = {path: path,
      ext: ext,
      thumbs: 2
    };
    processImage(img, props, fields, next, false, 'higheraltitude.prism');
  };
  var user = req.user;
  var date = moment().format('ssmmhhDDMMYYY');
  var ext = '.jpg';
  var path = date + uuid.v1();
  var form = new multiparty.Form();
  var onComplete = function(err, path, fields){
    var url = 'https://s3.amazonaws.com/higheraltitude.prism/' +
      path;
  ...
  form.parse(req, function(err, fields, files) {
    if (files) {
      var img = files.image;
      if (img.length > 0 && img[(img.length -1)].size > 0) {
        uploadImage(img[(img.length - 1)], path, ext, fields, onComplete);
      } else {
        next(err, false, fields);
      }
    } else next(err, false, fields);
  });

}

exports.uploadWelcome = function(req, next){
  var uploadImage = function(img, path, ext, fields, next){
    var props = {path: path,
      ext: ext,
      thumbs: 2,
    ...
    processImageRaw(img, props, fields, next, false, 'higheraltitude.prism');
  };
  var user = req.user;
  var date = moment().format('ssmmhhDDMMYYY');
  var ext = '.jpg';
...
exports.uploadInsight = function (req, next){
  console.log('uploading insight');
  var uploadImage = function(img, path, ext, fields, next){
    var props = {path: path,
      ext: ext,
      thumbs: 2
    };
    processImage(img, props, fields, next, false, 'higheraltitude.prizm.insights');
  };
  var user = req.user;
  var date = moment().format('ssmmhhDDMMYYY');
  var ext = '.jpg';
  var path = date + uuid.v1() + '-insight';
  var form = new multiparty.Form();
  var onComplete = function(err, path, fields){
    var url = 'https://s3.amazonaws.com/higheraltitude.prizm.insights/' +
      path;
    next(err, url, fields);
  };
  form.parse(req, function(err, fields, files) {
...
exports.uploadAvatar = function(req, next) {
 var uploadImage = function (img, path, ext, fields, next) {
    var props = {
      path: path,
      ext: ext,
  ...
  form.parse(req, function(err, fields, files) {
    var img = files.avatar;
    if (!img) img = files.image;
    if (img.length > 0 && img[0].size > 0){
      uploadImage(img[0], path, ext, fields, onComplete); 
    } else {
      next(err, false, fields);
...
exports.uoadImage = function(req, res, settings, next) {
  var s3 = new AWS.S3();
  var form = new multiparty.Form();
  var date = moment().format('ssmmhhDDMMYYYY');
  var unique = uuid.v1();
  ...
  form.parse(req, function (err, fields, files) {
    var imageUrl  = 'https://s3.amazonaws.com/' + AWS_BUCKET 
                    + '/' + folder + fileName;
    var targetWidth = Number(fields.width || 0) > 0?Number(fields.width) * 2:600;
    var targetHeight = Number(fields.height || 0) > 0?Number(fields.height) *2: 600;
    if (files) {
      var fa = files.image;
      if (fa) {
        var file;
