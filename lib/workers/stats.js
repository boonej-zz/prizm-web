                    ...
                    fetchInsightCount(o.owner._id, start, function(err, c, t){
                      ...
                      fetchInsightCount(o.owner._id, lastStart, function(err, c, t){
                        ...
                        fetchTrendingTags(users, lastStart, function(err, tags){
                          ...
                          fetchTrendingPost(users, start, function(err, m){
                            s.trendingPost = m;
                            next(s, o);
                          });
                        }, start);
...
var fetchMostPopularPost = function(u, start, next){
  var users = _.pluck(u, '_id');
  Activity.aggregate(
    {$match: {to: {$in: users}, action: {$in: ['like', 'comment']}, post_id: {$ne: null}, create_date: {$gt: start}}},
...
var fetchTrendingPost = function(u, start, next){
  var users = _.pluck(u, '_id');
  Activity.aggregate(
    {$match: {from: {$in: users}, action: {$in: ['like', 'comment']}, post_id: {$ne: null}, create_date: {$gt: start}}},
    {$group: {_id: '$post_id', count: {$sum: 1}}},
    {$sort: {count: -1}},
    {$limit: 1}
  )
